...
test('works ont imeout error', async (t) => {
    t.is((await got(`${s.url}/knock-twice`, {timeout: 100})).body, 'who`st here?');
});
...
test('function gets iter count', async (t) => {
    await got(`${s.url}/fifth`, {timeout: 100, retries: (iter:number, err) => {
        return iter < 10 ? 1 : 0
    }});
    t.is(fifth, 6);
});
